...
if __name__ == "__main__":
    ...
    # ponto, deve ser um numero par
    M = int(input("Tamanho do filtro: "))

    i, w = hamming(sample_rate, M)

    # plota
    plt.figure(u"Gráficos", figsize=(15, 8))
    plt.subplot(2, 1, 1)
    plt.title(u"Gráfico Hamming tempo")
    plt.xlabel("amostra")
    ...
    plt.subplot(2, 1, 2)
    plt.title(u"Gráfico Hamming frequencia (dB)")
    plt.plot(w, 20 * np.log10(abs(h)), 'b')
    plt.ylabel("Magnitude")

    plt.tight_layout()
    plt.savefig("Hamming.png", format="png")
